...
export function StudentCard({ student, onClick }: { student: Student; onClick?: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group flex w-full items-center gap-3 rounded-2xl border border-slate-100 bg-white p-3 text-left shadow-sm ring-1 ring-slate-900/5 transition-all hover:border-blue-100 hover:bg-blue-50/40 hover:shadow-md active:scale-[0.99]"
    >
      <div className="h-14 w-14 shrink-0 overflow-hidden rounded-full bg-blue-50 ring-2 ring-white">
        {student.avatarUrl ? (
          <img
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex items-center gap-1.5 text-xs">
          <span className="font-medium text-blue-600">{student.className}</span>
          <span className="text-slate-300">•</span>
          <span className="text-slate-400">លេខរៀង {toKhmerNumeral(student.rollNumber)}</span>
        </div>
      </div>

      <ChevronRight className="h-5 w-5 shrink-0 text-slate-300 transition-transform group-hover:translate-x-0.5 group-hover:text-blue-400" />
    </button>
  );
}